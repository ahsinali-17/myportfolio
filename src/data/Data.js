export const Bio = {
  name: "M. Ahsin Ali",
  gender: "Male",
  age: 21,
  married: false,
  location: {
    city: "Wah Cantt",
    country: "Pakistan",
    district: "Rawalpindi",
    postalCode: "47040",
    province: "Punjab",
    timezone: "GMT+5",
    // address: "<iframe src=\"https://www.google.com/maps/embed?pb=!1m17!1m12!1m3!1d3315.8508719120705!2d72.716082!3d33.790349!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m2!1m1!2zMzPCsDQ3JzI1LjMiTiA3MsKwNDInNTcuOSJF!5e0!3m2!1sen!2s!4v1759232196734!5m2!1sen!2s\" width=\"600\" height=\"450\" style=\"border:0;\" allowfullscreen=\"\" loading=\"lazy\" referrerpolicy=\"no-referrer-when-downgrade\"></iframe>",
  },
  email: "aahsinali17@gmail.com",
  phone: +923126627792,
  DOB: "17th August, 2004",
  roles: [
    "MERN Stack Developer",
    "Web App. Developer",
    "Next Js Developer",
    "AI Enthusiast",
  ],
  description:
    "I am a motivated and adaptable individual who values learning and tackling new challenges. Focused on delivering quality results, I approach tasks with a positive attitude and a growth mindset, aiming to contribute effectively and achieve meaningful outcomes.",
  github: "https://github.com/ahsinali-17",
  resume: "https://drive.google.com/file/d/1-VA2baIFnhiibv3yIQDh_AIvVJi7SSxQ/view?usp=drive_link",
  linkedin: "https://www.linkedin.com/in/ahsin-ali-3a5135276/",
  twitter: "https://x.com/Ahsinali_17?t=Ebsaxzwff1yVlPHuMoc9jQ&s=09",
  insta: "https://www.instagram.com/ahsinali_17/",
  facebook: "https://www.facebook.com/profile.php?id=100023936348059",
};

export const skills = [
  {
    title: "Languages",
    skills: [
      {
        name: "JavaScript",
        image: "https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/250px-Unofficial_JavaScript_logo_2.svg.png?utm_source=commons.wikimedia.org&utm_campaign=parser&utm_content=thumbnail",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "TypeScript",
        image: "https://cdn.worldvectorlogo.com/logos/typescript.svg",
        skillLevel: 85,
        skillExperience: "1 year",
      },
      {
        name: "C++",
        image: "https://i.pinimg.com/736x/c2/66/4d/c2664dcf18d2fd777aa954df6cd113a2.jpg",
        skillLevel: 80,
        skillExperience: "2 years",
      },
      {
        name: "Java",
        image: "https://images.seeklogo.com/logo-png/15/2/java-logo-png_seeklogo-158094.png",
        skillLevel: 75,
        skillExperience: "1.5 years",
      },
    ],
  },
  {
    title: "Frontend",
    skills: [
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
        skillLevel: 95,
        skillExperience: "2+ years",
      },
      {
        name: "CSS",
        image: "https://coryrylan.com/assets/css-CfW5UZaa.svg",
        skillLevel: 95,
        skillExperience: "2+ years",
      },
      {
        name: "React Js",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSg1MndL-Xp1JcnqaB0YOqTp6zDjrwYyGKsPA&s",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "Next Js",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSV9uzErWz9EXqZDxZ5lP9aYpMz8eK6rr5X3w&s",
        skillLevel: 85,
        skillExperience: "1.5 years",
      },
      {
        name: "Tailwind CSS",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQDML5CFq70Y9FJ52YnyCjfdyUA3g9B6is_jA&s",
        skillLevel: 95,
        skillExperience: "2 years",
      },
      {
        name: "Redux - Toolkit",
        image: "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
        skillLevel: 90,
        skillExperience: "1.5 years",
      },
      {
         name: "Zustand",
        image: "https://user-images.githubusercontent.com/958486/218346783-72be5ae3-b953-4dd7-b239-788a882fdad6.svg",
        skillLevel: 80,
        skillExperience: "1.5 years",
      },
      {
        name: "GSAP",
        image: "https://miro.medium.com/v2/resize:fit:1100/format:webp/1*wGdP-ym3kqQopOA9us8nXg.jpeg",
        skillLevel: 85,
        skillExperience: "1 year",
      },
      {
         name: "motion-animation lib.",
        image: "https://user-images.githubusercontent.com/7850794/164965509-2a8dc49e-2ed7-4243-a2c9-481b03bbc31a.png",
        skillLevel: 90,
        skillExperience: "1 year",
      },
      {
        name: "React Native",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSg1MndL-Xp1JcnqaB0YOqTp6zDjrwYyGKsPA&s",
        skillLevel: 85,
        skillExperience: "1 year",
      }
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
        skillLevel: 85,
        skillExperience: "2 years",
      },
      {
        name: "Express Js",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ7S33Oq2FeRbyBBA6l1q8PwLVa3SzaONO-9Q&s",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "Postgresql",
        image: "https://www.postgresql.org/media/img/about/press/elephant.png",
        skillLevel: 80,
        skillExperience: "1 year",
      },
      {
        name: "MongoDB",
        image: "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
        skillLevel: 85,
        skillExperience: "2 years",
      },
      {
        name: "Supabase",
        image: "https://images.seeklogo.com/logo-png/43/1/supabase-logo-png_seeklogo-435677.png",
        skillLevel: 90,
        skillExperience: "1 year",
      },
          {
        name: "Socket.io",
        image: "https://avatars.githubusercontent.com/u/10566080?v=4",
        skillLevel: 85,
        skillExperience: "1 year",
      },
       {
        name: "NestJs",
        image: "https://www.vhv.rs/dpng/d/498-4989583_nestjs-logo-hd-png-download.png",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "Stripe",
        image: "https://www.citypng.com/public/uploads/preview/hd-stripe-official-logo-png-701751694777755j0aa3puxte.png",
        skillLevel: 80,
        skillExperience: "1 year",
      },
    ],
  },
  {
    title: "Other Tools",
    skills: [
      {
        name: "Git",
        image: "https://git-scm.com/images/logos/downloads/Git-Icon-1788C.png",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "GitHub",
        image: "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
        skillLevel: 90,
        skillExperience: "2 years",
      },
      {
        name: "VS Code",
        image: "https://w7.pngwing.com/pngs/512/824/png-transparent-visual-studio-code-hd-logo-thumbnail.png",
        skillLevel: 95,
        skillExperience: "2+ years",
      },
         {
        name: "Cursor",
        image: "https://upload.wikimedia.org/wikipedia/commons/e/e1/Cursor_logo.png?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=original",
        skillLevel: 95,
        skillExperience: "2+ years",
      },
      {
        name: "Postman",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWeEbxr2jaJP6Y3GKedSkC-nwBiblIG5USaQ&s",
        skillLevel: 95,
        skillExperience: "1.5 years",
      },
      {
        name: "Langchain",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyVr9cLs6MgRqhnkGKpSHrY8IIjrZjrnAcOMU3pfleebudD6wfXaAi2iAS&s=10",
        skillLevel: 90,
        skillExperience: "1 year",
      },
    ],
  },
];

export const experience = [
  {
    id: 0,
    img: "/exp_edu/blink.jpeg",
    role: "Web Developer Intern",
    company: "Blink Consolidate",
    date: "April 2025 - July 2025",
    desc: "Completed a remote internship at Blink Consolidate as a full stack web developer. As an intern, I built responsive UIs with Next.js, React, and Tailwind CSS. Created and connected REST APIs to backend and database for dynamic content and authentication. Handled routing, form interactions, and hosted complete applications on VPS.",
    skills: [
      "NextJS",
      "ReactJS",
      "PostgreSQL",
      "NodeJS",
      "ExpressJS",
      "Tailwind CSS",
      "Git",
    ],
    doc: "https://drive.google.com/file/d/1fzE4oHycK4rHI_aknOTmaR63Onf78FFv/view?usp=drive_link",
  },

  {
    id: 1,
    img: "/exp_edu/internee.png",
    role: "Frontend Developer Intern",
    company: "Internee.pk",
    date: "May 2024 - July 2024",
    desc: "Completed a remote internship at internee.pk as a React.js developer, focusing on creating responsive web interfaces and integrating APIs for enhanced user experience.",
    skills: [
      "ReactJS",
      "Redux - Toolkit",
      "Firebase",
      "Tailwind CSS",
      "HTML",
      "CSS",
      "JavaScript",
    ],
    doc: "https://drive.google.com/file/d/1eiHMqYWBtTSQm34lPhQe398jYDMla5TY/view?usp=drive_link",
  },
  
];

export const education = [
  {
    id: 0,
    img: "/exp_edu/uet.jpg",
    school: "University Of Engineering & Technology, Taxila",
    date: "Oct 2022 - August 2026",
    grade: "3.57 CGPA",
    desc: "I have completed my graduation degree in Computer Science at the University of Engineering & Technology, Taxila, with an overall CGPA of 3.57/4.00. My coursework includes Data Structures, Algorithms, OOP, Databases, Operating Systems, AI/ML, Artificial Neural Networks, Compiler Construction, and Computer Networks.",
    degree: "Bachelor of Science in Computer Science",
  },
];

export const certifications = [
  {
    id: 0,
    img: "/certificates/MERN.png",
    school: "The Great Learning Academy" ,
    date: "Feb 2025",
    desc: "Successfully completed the MERN Stack Fundamentals course, acquiring foundational knowledge of MongoDB, Express.js, React.js, and Node.js for building dynamic web applications.",
    title: "MERN Stack Fundamentals Certificate",
  },
  {
    id: 1,
    img: "/certificates/genAI.jpeg",
    school: "NCEAC" ,
    date: "May 2026",
    desc: "Earned the Generative AI Application Developer Certificate from UETIANS Lahore Endowment Foundation (USA) in collaboration with HEC and PAKANGELS. Graduated as a Top Performer in Cohort 3.",
    title: "Generative AI Application Developer Certificate",
  },
];

export const projects = [
  {
    id: 0,
    title: "ScreenSenseAI-FYP",
    isAi: true,
    description: "An AI-powered screen defect detection system that utilizes computer vision and machine learning algorithms to identify and classify screen defects in real-time, providing accurate and efficient quality control for manufacturing processes.",
    image: "/projects/screenSenseAI.jpg",
    tags: ["React Js", "Computer Vision", "YOLOv12", "Tailwind CSS", "Express Js","Flask", "Supabase", "jsPDF", "Tanstack"],
    category: "web app, AI",
    link: "https://fyp-frontend-zeta.vercel.app/",
    features: ["Real-time AI Defect Detection", 
      "Secure PDF Audit Generation", 
      "Batch Image Processing", 
      "LiveImage Scanning"],
    challenges: "Fine-tuning a custom Detection Transformer and YOLO model, ensuring frontend state integrity via TanStack Query; optimizing PDF report generation speed using parallel fetching, resolving dependency conflicts in headless Docker deployments.",
    duration: "6 months",
  },
  {
    id: 1,
    title: "Employeed",
    description:
      "A full stack job portal app that enables users to search and apply for job openings, featuring a responsive UI, filterable listings, and an application drawer built using supabase, clerk, React and shadcn.",
    image: "/projects/employeed.jpg",
    tags: ["React Js", "Supabase", "Clerk", "Shadcn", "zod"],
    category: "web app",
    link: "https://employeed.vercel.app/",
    features: ["Job search", "Filterable listings", "User authentication", "Application management"],
    challenges: "Integrating Supabase with Clerk for authentication and handling dynamic queries efficiently.",
    duration: "6 weeks",
  },
   {
    id: 2,
    title: "DocMind AI",
    isAi: true,
    description: "A context-aware AI agent using Retrieval-Augmented Generation (RAG) that enables users to query complex documents and verify answers through a synchronized split-screen interface.",
    image: "/projects/docmindAI.jpg",
    tags: ["React Js", "RAG", "MongoDb", "Express js", "Vector Embeddings"],
    category: "web app, AI",
    link: "https://github.com/ahsinali-17/docMind-AI",
    features: ["Real-time AI Defect Detection", 
      "Secure PDF Audit Generation", 
      "Batch Image Processing", 
      "Gemini AI Image Validation"],
    challenges: "Synchronizing the PDF viewer state with AI-generated citations for precise 'click-to-scroll' navigation and managing asynchronous data streams.",
    duration: "1 weeks",
  },
  {
    id: 3,
    title: "connectify",
    description:
      "Built a modern, feature-rich social media platform using MERN. The app includes functionalities such as user authentication, profile management, content creation (posts, images, and videos) and interactive features like likes, comments, add friend and many more With a clean and responsive UI",
    image: "/projects/connectify.jpg",
    tags: ["React Js", "MongoDb", "Node Js", "Express Js", "Redux - Toolkit", "Redux-persist", "Tailwind CSS", "Multer", "NodeMailer"],
    category: "web app",
    link: "https://github.com/ahsinali-17/connectify",
    features: ["User authentication", "Profile management", "Post creation", "Media uploads", "Likes & comments", "Friend system"],
    challenges: "Optimizing state management with Redux-persist and handling media uploads securely.",
    duration: "1 month",
  },
  {
    id: 4,
    title: "BackMyProjects",
    description: "A crowdfunding Next.js app, featuring Razorpay payment gateway integration for secure transactions.",
    image: "/projects/backMyProject.jpg",
    tags: ["Next Js", "Tailwind CSS", "MongoDb", "Stripe"],
    category: "web app",
    link: "https://back-my-projects.vercel.app/",
    features: ["Project listings", "Payment gateway integration", "Responsive design"],
    challenges: "Integrating Stripe with Next.js API routes and ensuring secure transactions.",
    duration: "2 weeks",
  },
  {
    id: 5,
    title: "Uber-clone",
    description: "Uber-clone is a modern ride-booking web app with real-time location tracking, user authentication, and responsive design.",
    image: "/projects/uber.webp",
    tags: ["React Js", "Express Js", "MongoDB", "Tailwind CSS", "Socket.io", "GoMaps api"],
    category: "web app",
    link: "https://github.com/ahsinali-17/uber-clone",
    features: ["Real-time ride booking", "Location tracking", "Authentication", "Responsive UI"],
    challenges: "Implementing real-time ride updates with Socket.io and integrating maps API effectively.",
    duration: "6 weeks",
  },
  {
    id: 6,
    title: "StripeStore",
    description: "StripeWalaStore is a modern e-commerce web app with secure Stripe payments, real-time user authentication, and responsive design.",
    image: "/projects/stripeStore.jfif",
    tags: ["React Js", "Firebase", "Express Js", "Tailwind CSS", "Stripe", "Redux-Toolkit", "Redux-persist"],
    category: "web app",
    link: "https://stripe-store-lake.vercel.app/",
    features: ["Stripe payments", "User authentication", "Product management", "Cart & checkout"],
    challenges: "Integrating Stripe securely with Firebase authentication and managing cart state with Redux.",
    duration: "2 weeks",
  },
  {
    id: 7,
    title: "Password manager",
    description: "A react app which can remember provided passwords along with the username and website's URL...",
    image: "/projects/passwordManager.jpg",
    tags: ["React Js", "MongoDb", "Node Js", "Express Js", "Tailwind CSS"],
    category: "web app",
    link: "https://github.com/ahsinali-17/Password-Manager",
    features: ["Save credentials", "Edit & delete", "Copy password", "MongoDB storage"],
    challenges: "Ensuring secure storage and retrieval of credentials, handling encryption.",
    duration: "3 weeks",
  },
 
  {
    id: 8,
    title: "Spotify Clone",
    description: "Spotify clone with a functional music player and responsive audio controls.",
    image: "/projects/spotify.jpg",
    tags: ["HTML", "CSS", "JavaScript"],
    category: "web app",
    link: "https://github.com/ahsinali-17/Spotify-clone",
    features: ["Music player", "Album browsing", "Responsive design"],
    challenges: "Handling audio playback and managing UI interactions with vanilla JS.",
    duration: "3 weeks",
  },
    {
    id: 9,
    title: "WealthBrain AI",
    isAi: true,
    description: "An AI-powered cross-platform financial tracking and intelligent budget management mobile app developed with React Native and Expo. Integrates AI financial analytics, automated budget forecasting, income & expense logging, interactive charts, and transaction history.",
    image: "/projects/wealth-manager.png",
    tags: ["React Native", "Expo Router", "Gemini SDK", "Zustand", "Supabase", "Tanstack Query","Nativewind CSS", "React Native Reanimated", "Expo Animations"],
    category: "mobile app, AI",
    link: "https://github.com/ahsinali-17/wealth-manager-mobile-app",
    features: ["AI Financial Insights & Analytics", "Income & Expense Tracking", "Interactive Financial Charts", "Smart Budget Category Breakdown", "Cloud Transaction Sync"],
    challenges: "Building high-performance AI analytics chart visualizers with real-time state updates across mobile viewports.",
    duration: "4 weeks",
  },
  {
    id: 10,
    title: "Get Roofed",
    description: "A feature-rich real estate mobile application built using React Native and Expo. Enables users to explore property listings, filter by location & amenities, manage favorites, and authenticate securely with Clerk and Supabase.",
    image: "/projects/get-roofed.png",
    tags: ["React Native", "Expo Router", "Supabase", "Clerk", "AsyncStorage", "Nativewind CSS"],
    category: "mobile app",
    link: "https://github.com/ahsinali-17/get-roofed",
    features: ["Property Discovery & Filters", "User Auth via Clerk", "Saved & Bookmarked Properties", "Supabase Database Integration"],
    challenges: "Structuring cross-platform navigation with Expo Router, implementing secure state persistence with AsyncStorage, and integrating Clerk authentication with Supabase RLS policies.",
    duration: "3 weeks",
  },
  {
    id: 11,
    title: "G72 Studio Clone",
    description: "A high-performance interactive web page replicating the aesthetic G72 Studio design. Built with vanilla HTML/CSS and JavaScript, featuring smooth Locomotive scrolling, GSAP micro-animations, and dynamic visual interactions.",
    image: "/projects/g72.png",
    tags: ["HTML", "CSS", "JavaScript", "GSAP", "Locomotive Scroll"],
    category: "animation",
    link: "https://g72-clone.vercel.app/",
    features: ["Locomotive Physics Scroll", "GSAP ScrollTrigger Micro-animations", "Interactive Hover Dynamics", "Responsive Dark Theme Design"],
    challenges: "Calibrating smooth scroll parameters with Lenis/Locomotive scroll, ensuring 60fps frame rate during scroll transitions, and handling cross-browser CSS transform compatibility.",
    duration: "1 week",
  },
  {
    id: 12,
    title: "Sundown Studio Clone",
    description: "A pixel-perfect clone of the award-winning Sundown Studio website. Built with rich static content, dynamic GSAP animation timelines, custom cursor movements, fluid page transitions, and media sliders.",
    image: "/projects/sundown.png",
    tags: ["HTML", "CSS", "JavaScript", "GSAP", "Swiper.js"],
    category: "animation",
    link: "https://sundown-clone-sand.vercel.app/",
    features: ["Custom Following Cursor Effect", "GSAP Timeline Animations", "Swiper.js Interactive Sliders", "Fluid Liquid Gradient Accents"],
    challenges: "Implementing complex hardware-accelerated animations using GSAP and creating custom interactive pointer events without layout thrashing.",
    duration: "1 week",
  },
   {
    id: 13,
    title: "Wordle-clone",
    description: "Wordle-clone is an interactive web-based word puzzle game built with React and custom feedback logic.",
    image: "/projects/wordle-clone.png",
    tags: ["React Js", "Tailwind CSS", "JavaScript", "Game Logic"],
    category: "game",
    link: "https://wordle-opal-xi.vercel.app",
    features: ["Word puzzle gameplay", "Color-coded feedback system", "Responsive design"],
    challenges: "Managing word validation algorithms and real-time state feedback efficiently.",
    duration: "1 week",
  },
  {
    id: 14,
    title: "Tic Tac Toe Game",
    description: "An interactive, web-based Tic Tac Toe game built with React. Features single-player mode against an AI opponent powered by the Minimax algorithm, 2-player mode, sound effects, and win/draw animations.",
    image: "/projects/tic-tac-toe.png",
    tags: ["React Js", "Tailwind CSS", "JavaScript", "Minimax AI", "Game Logic"],
    category: "game",
    link: "https://tic-tac-toe-sigma-nine-97.vercel.app/",
    features: ["Minimax AI Opponent", "2-Player Pass & Play", "Smooth Win/Draw Animations", "Scoreboard & Sound Effects"],
    challenges: "Implementing an optimal Minimax decision-tree algorithm for the AI bot while keeping game state updates fluid and responsive.",
    duration: "1 week",
  },
];
